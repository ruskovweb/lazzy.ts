...
export interface ILazyCollectionAsync<T, R, N> {
    ...
    flatMap<V, D extends Depth = 20>(transformer: (currentValue: T, index: number) => V, depth?: D): ILazyCollectionAsync<FlatArray<V, D>, R, undefined>;
    forEach(fun: (v: T, i: number) => void | Promise<void>): ILazyCollectionAsync<T, R, undefined>;
    groupBy<TKey, TElement, TResult>(keySelector: (v: T) => TKey, elementSelector: (v: T) => TElement, resultSelector: (key: TKey, elements: TElement[]) => TResult): ILazyCollectionAsync<TResult, R, undefined>;
